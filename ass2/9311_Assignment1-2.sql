...
create table Places (
	...
	postalcode  integer,
	gpscoords	LocationValue unique, ------------------------bug
...
create table RepeatingEvents (
	id          serial,
	eventinfo_id integer,
	lowerDate   date not null,
	upperDate   date not null check (lowerDate < upperDate),
	event_id     serial,
    EventinfoId      serial,
	primary key (id)
);
...
------eventinfo
alter table EventInfo add constraint	eventinfo_places foreign key (location) references Places(gpscoords);
...
-- -------- insert theme
-- insert into PageColours(owner) values(1),(2),(3);
...
-- -----soldtick
-- insert into SoldTickets(event_id,quantity,ticket_type,purchaser)
-- values (1,1,1,5),
-- (1,2,1,2),
-- (2,1,2,1),
-- (4,3,4,2),
